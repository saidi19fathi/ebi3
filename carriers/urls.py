...
urlpatterns = [
    # REDIRECTIONS pour les URLs d'inscription qui ont été déplacées vers users/
    # Ces redirections permettent de ne pas casser les anciens liens
    path('register/', RedirectView.as_view(
        pattern_name='users:carrier_register',
        permanent=True
    ...
    path('logout/', RedirectView.as_view(
        pattern_name='users:logout',
        permanent=True
    ), name='logout_redirect'),

    # 1. DASHBOARD ET PROFIL TRANSPORTEUR
    ...
    # 6. MARKETPLACE (offres et recherche)
